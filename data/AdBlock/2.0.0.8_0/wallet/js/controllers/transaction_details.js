...
thinwalletCtrls.controller('TransactionDetailsCtrl', function ($scope,
    ...
    ApiCalls.get_tx(address, view_key, tx_hash)
        .then(function(response) {
            ...
            // $scope.formatedTime     = new Date(data.timestamp).toLocaleString().split('/');
            // if ( $scope.formatedTime[0] < 10){
            //     $scope.formatedTime[0] = '0' + $scope.formatedTime[0];
            // }
            // $scope.formatedTime = $scope.formatedTime[1] + '.' + $scope.formatedTime[0] + '.' + $scope.formatedTime[2];
                ...
                if (view_only === false) {
                    for (var j = 0; j < data.spent_outputs.length; ++j) {
                        ...
                        if (data.spent_outputs[j].key_image !== key_image) {
                            total_sent = new BigInteger(total_sent).subtract(data.spent_outputs[j].amount);
            ...
            $scope.tx_amount = new BigInteger(total_received || 0).subtract(total_sent || 0).toString();
