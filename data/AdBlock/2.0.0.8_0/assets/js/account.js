...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.getElementById('profileImageIcon').src = localStorage.getItem('image');
        ...
        switch (type) {
          case 'nationalId':
            if (localStorage.getItem('document1') == '') {
              await firebase.storage().ref().child('userdocuments/' + 'Document-1-' + firebase.auth().currentUser.uid).putString(image1, 'data_url');
              urlImage1 = await firebase.storage().ref().child('userdocuments/' + 'Document-1-' + firebase.auth().currentUser.uid).getDownloadURL();
              localStorage.setItem('document1', urlImage1);
              data.document1URL = urlImage1;
            }

            if (localStorage.getItem('document2') == '') {
              await firebase.storage().ref().child('userdocuments/' + 'Document-2-' + firebase.auth().currentUser.uid).putString(image2, 'data_url')
              urlImage2 = await firebase.storage().ref().child('userdocuments/' + 'Document-2-' + firebase.auth().currentUser.uid).getDownloadURL();
  ...
  const uploadUserSelfie = async () => {
    try {
      if (localStorage.getItem('selfie') == '') {
        await firebase.storage().ref().child('userdocuments/' + 'Selfie-' + firebase.auth().currentUser.uid).putString(selfie, 'data_url');
        const urlSelfie = await firebase.storage().ref().child('userdocuments/' + 'Selfie-' + firebase.auth().currentUser.uid).getDownloadURL();
        const fullName = document.getElementById('fullName').value;
        const data = {
          selfie: urlSelfie,
          real_name: fullName,
  ...
  function showConfirmTransaction() {
    ...
    document.getElementById('spanAmount').innerText = amount + ' <i class="icon-font-tube tubeFont10"></i> + fee';
      ...
      onBeforeShow: function (ev, inst) {
        switch (status) {
          ...
          case 'finishProcess':
            ...
            break;
          case 'startProcess':
            document.getElementById('textUserModal').innerHTML = i18next.t('startVerification');
            break;
        }
      }
    });
...
function processing(spent_outputs, getAddressInfo, total_received, total_received_unlocked) {
  ...
  if (spent_outputs.length > 0) {
    setTimeout(processing(spent_outputs, getAddressInfo, total_received, total_received_unlocked), 100);
  } else {
    loadAvailBal(getAddressInfo, total_received, total_received_unlocked);
  }
}
