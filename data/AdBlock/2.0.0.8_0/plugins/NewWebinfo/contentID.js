...
async function getContentIDImpl() {
  ...
  // Amazon
    ...
    if (document.getElementById("bylineInfo") != undefined) { // Store page
      ...
      // console.log('Amazon 5 Author Profile -', retVal.userID, retVal.userName);
    } 
    // else if (document.getElementsByClassName("brand-store-title")[0] != undefined) {
    //   retVal.userName = document.getElementsByClassName("brand-store-title")[0].innerText; // StoreName
    //   // retVal.userID = document.getElementsByClassName("brand-store-title")[0].baseURI; // StoreHref
    //   console.log('Amazon 2', retVal.userID, retVal.userName);
    // } 
    // else if (document.getElementById("ap-author-name") != undefined) {
    //   retVal.userName = document.getElementById("ap-author-name").innerText; // StoreName
    //   // retVal.userID = document.getElementById("ap-author-name").baseURI; // StoreHref
    //   console.log('Amazon 3', retVal.userID, retVal.userName); // Movie video prime store page
    // } 
    // else if (document.getElementsByClassName("av-detail-section")[0] != undefined) {
    //   retVal.userName = document.getElementsByClassName("av-detail-section")[0].children[0].innerText; // MovieName
    //   retVal.userID = document.querySelector('[data-title-id]').getAttribute('data-title-id');
    //   console.log('Amazon 4 Video -', retVal.userID, retVal.userName);
    // }
    // if (!retVal.userName) { retVal.userName = retVal.userID; }
    if (retVal.userID) {
  ...
  // Twitch
    ...
    try {
      if (url.indexOf("/videos") > -1) {
        ...
        if (elements[0] != undefined) {
          retVal.userID = elements[0].baseURI.split("twitch.tv/")[1].split('/')[0];
          retVal.userName = elements[0].parentElement.innerText.trim();
          if (retVal.userName == "") { // Silly twitch having placeholders.
            retVal.userName = retVal.userID;
          }
        }
  ...
  // Twitter
    ...
    if ( elements.length === 0 ){
      elements = document.querySelector('.css-1dbjc4n.r-15d164r.r-1g94qm0 > .css-1dbjc4n.r-1wbh5a2.r-dnmrzs > .css-1dbjc4n.r-18u37iz.r-1wbh5a2>div>span');
      if ( elements != null ){
    ...
    if (url.indexOf("channel") > -1) {
      ...
      if ( channelTitleElem != null ){
        ...
        setTimeout(function(e){
          if ( document.querySelectorAll('#channel-title-container > #channel-title')[0] != undefined ){
