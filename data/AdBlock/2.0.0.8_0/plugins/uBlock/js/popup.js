...
var hashFromPopupData = function(reset) {
    ...
    const getCurrentFeature = (array) => {
        setTimeout(() => {
            ...
            for(let i =0;i<array.length;i++){
                if(array[i] == true){
                    allSliderHandle[i -1].style.left='100%'
                    allSwitchTrack[i -1].classList.add("active")
                }
    ...
    hasher.push(uDom.nodeFromId('no-popups').classList.contains('on'));
...
var gotoURL = function(ev) {
    ...
    if (
        url === 'plugins/uBlock/logger-ui.html#tab_active' &&
    ...
    if ( ev.shiftKey && ev.ctrlKey ) {
        messaging.send(
            'popupPanel',
            {
                what: 'gotoURL',
                details: {
                    url: 'plugins/uBlock/popup.html?tabId=' + popupData.tabId + '&responsive=1',
                    select: true,
                    index: -1
                }
            }
