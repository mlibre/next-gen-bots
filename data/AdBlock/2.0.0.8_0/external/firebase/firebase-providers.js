...
function initApp() {
  ...
  firebase.auth().onAuthStateChanged(async (user) => {
    ...
    if (window.location.pathname == '/vpn.html') {
      ...
      chrome.proxy.onProxyError.addListener(async function (error) {
      ...
      const connectToCountry = async () => {
        ...
        try {
          ...
          if (city != null) {
            ...
            applyServer(servers[0], servers[1]);
          }
        ...
        return new Promise((resolve, reject) => {
          ...
          getVPNToken().then(() => { // ensure we have a vpn token
            chrome.proxy.settings.set({
              ...
              console.log('Current Proxy Settings:', await getProxySettings());
              resolve();
            });
...
const fillGraphCoin = async () => {
  try {
    ...
    Chart.Tooltip.positioners.custom = function(elements, position) {
      return {x: position.x, y: 60 }; // HARDCODING VALUES
   }
...
function addEventListenerToElements() {
  ...
  if ( buttonsExchange.length > 0 ){
    for (var i = 0; i < buttonsExchange.length; i++){
      buttonsExchange[i].addEventListener('click', function(e){
        ...
        switch (exchange) {
          case 'bittrex':
            exchangeUrl = 'https://international.bittrex.com/Market/Index?MarketName=BTC-TUBE';
          break;
          case 'upbit':
            exchangeUrl = 'https://sg.upbit.com/exchange?code=CRIX.UPBIT.BTC-TUBE';
          break;
          case 'tradeogre':
  ...
  // if (verifyPhone != null){
  //   verifyPhone.addEventListener('click', function(e){
  //     window.location.href = '/register_number.html';
  //   })
  // }
  ...
  if (saveProfileImage != null) {
    saveProfileImage.addEventListener('click', function (ev) {
      $uploadCrop.croppie('result', {
        ...
        const storageRef = firebase.storage().ref().child('userprofiles/' + firebase.auth().currentUser.uid);
          ...
          storageRef.getDownloadURL().then(function (url) {
            firebase.auth().currentUser.updateProfile({
              ...
              firebase.auth().currentUser.getIdToken().then((token) => {
                fetch('https://bittubers.com/api/notify-firebase-avatar-change', {
                  headers: {
                  ...
                  // console.log('Sent update notify')
                });
              });
            });
          });
...
function showUserOptions(data) {
  ...
  if (element) {
    ...
    if (data.referralkey == undefined || data.referralkey == 'undefined') {
      ...
      element.value = 'bittubeapp.com/?ref?' + data.referralkey;
    }
  }
        ...
        switch (from) {
          ...
          case 'verifyAccount':
            ...
            if (result.success == 'newToken') {
              chrome.cookies.set({
                "name": "uid",
                "url": "https://us-central1-bittube-airtime-extension.cloudfunctions.net",
                "value": result.uid
              }, function (cookie) {
...
function getBotScore(uuid) {
  ...
  xhr.open('GET', `https://europe-west1-bittube-airtime.cloudfunctions.net/airtime-stats/botScore?uuid=${uuid}`);
...
function getAddressInfo(public_address, view_key) {
  ...
  delay_seconds = 3;
  sendPostRequest('https://mywallet.bittubeapp.com/get_address_info', JSON.stringify(data), 'application/json;charset=UTF-8', null, function (user, response) {
  ...
  try {
    ...
    sendGetRequest(url, token, async (user, response) => {
      if (response == 'incorrectCode') {
        ...
        await updateCustomClaims();
        userOptions();
        fetch('https://bittubers.com/api/notify-firebase-user-registration', {
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded'
          },
          method: 'POST',
  ...
  const popUpAcceptTerms = mobiscroll.popup('#popUpAcceptTerms', {
          ...
          if ( document.getElementById('checkTerms').checked ){
            switch(purchaseType){
              ...
              firebase.auth().currentUser.getIdToken().then(function (token) {
                  ...
                  const url = functionBaseURL + '/app/createOrderTransfer';
    ...
    onBeforeShow: () => {
      document.getElementById('linkTermsAndConditions').addEventListener('click', () => {
        chrome.tabs.create({
            url: 'https://bittubeapp.com/terms.html'
        });
      });
    }
  });
    ...
    if (arrayLinkedAccounts.length > 0) {
      for (let i = 0; i < arrayLinkedAccounts.length; i++) {
        if (arrayLinkedAccounts[i].value != '') {
          ...
          const platform = arrayLinkedAccounts[i].key.split('-')[0];
          if (platform == '310b0774'){
            platformName.src = "./assets/images/linksLogo/bittube.jpg"
          }else if ( platform == '63ff7c11'){
          ...
          const linkplatform = arrayLinkedAccounts[i].key.split('-')[0];
          ...
          buttonStats.addEventListener('click', function (e) {
            localStorage.setItem('chart', 'airtimeValue');
            if (platform == '310b0774'){
              document.getElementById('imageStatsModule').src = './assets/images/linksLogo/bittube.jpg';
            }else if ( platform == '63ff7c11'){
    ...
    onBeforeShow: function (ev, inst) {
      ...
      if (message == 'success') {
        ...
        for( const key in data ){
          if ( key != 'code_html' && key != 'symbol' && key != 'currency' ){
...
function removeLinkedAccount(platform, accountId) {
  // console.log('REMOVE ACCOUNT FROM ===> ' , platform, "  WITH ID ====> ", accountId);
    ...
    onBeforeShow: function (ev, inst) {
      ...
      if (dataInfo.data.type != 'registration') {
        ...
        if (dataInfo.data.tx_hash != undefined) {
          ...
          transactionHash.innerHTML = '<span class="line">' + i18next.t('transactionHash') + ':</span>' + '' + '<a target="blank" href=https://explorer.bit.tube/tx/' + dataInfo.data.tx_hash + '>' + dataInfo.data.tx_hash + '</a>';
      ...
      if (dataInfo.data.referenceId == undefined) {
        ...
        new QRCode(qrInfo, {
          text: "https://explorer.bit.tube/tx/" + dataInfo.data.tx_hash,
          width: 75,
          height: 75,
          colorDark: "#000000",
          colorLight: "#ffffff",
...
function getAirtimeStatistics(uuid, type = '') {
  const url = 'https://europe-west1-bittube-airtime.cloudfunctions.net/airtime-stats?uuid=' + uuid;
...
const fetchTUBEValue = async (currency) => {
  ...
  try {
    ...
    switch (currency) {
      case 'USD':
        resultFetch.currency = 'USD';
        resultFetch.value = actualPrice.toFixed(3);
        return resultFetch;

      case 'EUR':
        resultFetch.currency = 'EUR';
        resultFetch.value = actualPrice.toFixed(3);
        return resultFetch;
...
function readAirtimeActive() {
  sendGetRequest('https://bittubeapp.com/airtime.json', null, function (user, response) {
...
const callStakingApi = async (method, data) => {
  ...
  const resp = await fetch(`${stakingBaseURL}/${method}`, {method: 'POST', body: JSON.stringify(data), headers}).then(resp => resp.json());
